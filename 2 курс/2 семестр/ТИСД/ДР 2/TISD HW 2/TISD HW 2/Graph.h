...
namespace DTAS
{
	...
	public:
		...
		Graph(const Graph& graph) : _size(graph._size), _elements(graph._elements), _outputFile(graph._outputFile)
		{
			...
			for (int i{}; i < _size; ++i)
			{
				...
				for (int j{}; j < _size; ++j)
					if (graph._weights[i][j] != nullptr)
						_weights[i][j] = new int(*graph._weights[i][j]);
			}
		}

		Graph(uint16_t size, std::vector<T> elements) : _size(size), _elements(elements), _outputFile("Output.txt")
		{
			_weights = new int** [_size] {};
			for (int i{}; i < _size; ++i)
				_weights[i] = new int* [_size] {};
		...
		T GetElement(uint16_t index) { return _elements[index]; }

		bool CheckConnection(uint16_t i, uint16_t j) { return _weights[i][j] != nullptr; }

		bool GetWeight(uint16_t i, uint16_t j, int& weight)
		{
			if (_weights[i][j] == nullptr)
				return false;
		...
		void SetWeight(uint16_t i, uint16_t j, int* weight)
		{
			if (_weights[i][j] != nullptr)
				delete _weights[i][j];
				...
				if (index < _size)
				{
					...
					while (!waiting.empty())
					{
						...
						std::sort(unsorted.begin(), unsorted.end(), 
							[&](int a, int b) { return *_weights[index][a] < *_weights[index][b]; });
		...
		std::vector<uint16_t> FindShortestWay(uint16_t startIndex, uint16_t endIndex)
		{
			if (IsEmpty())
				return std::vector<uint16_t>{};

			bool* visited = new bool[_size] {};
			uint16_t* parents = new uint16_t[_size]{};
			std::queue<int> waiting{};
			...
			while (!waiting.empty())
			{
				...
				std::sort(unsorted.begin(), unsorted.end(),
					[&](int a, int b) { return *_weights[index][a] > *_weights[index][b]; });
				for (uint16_t i{}; i < unsorted.size(); ++i)
					waiting.push(unsorted[i]);
			}
